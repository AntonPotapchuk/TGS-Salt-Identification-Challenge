...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=1, downsample=None):
...
def make_layer(input_tensor, kernel_size, filters, blocks, stage, strides=1):
    downsample = None
    if strides != 1:
    ...
    x = conv_block(input_tensor, kernel_size, filters, stage, '1', strides, downsample)
    for i in range(1, blocks):
        x = conv_block(x, kernel_size, filters, stage, str(i + 1))
    return x
...
def ResNet34(include_top=True, input_shape=None,
    ...
    x = make_layer(x, (3, 3), 64, 3, 'a')
    x = make_layer(x, (3, 3), 128, 4, 'b', strides=2)
    x = make_layer(x, (3, 3), 256, 6, 'c', strides=2)
    x = make_layer(x, (3, 3), 512, 3, 'd', strides=2)
