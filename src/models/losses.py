...
def __flatten_binary_scores(scores, labels, ignore=None):
    """
...
def __lovasz_hinge(logits, labels, per_image=True, ignore=None):
    """
    Binary Lovasz hinge loss
    ...
    if per_image:
        def treat_image(log_lab):
            ...
            log, lab = __flatten_binary_scores(log, lab, ignore)
            return __lovasz_hinge_flat(log, lab)
        losses = tf.map_fn(treat_image, (logits, labels), dtype=tf.float32)
        loss = tf.reduce_mean(losses)
    else:
        loss = __lovasz_hinge_flat(*__flatten_binary_scores(logits, labels, ignore))
    return loss


def keras_lovasz_hinge(labels,logits):
    return __lovasz_hinge(logits, labels, per_image=True, ignore=None)
