...
from keras.layers import Concatenate, Conv2D, UpSampling2D, BatchNormalization, Activation, Conv2DTranspose, \
    Add, Input, ZeroPadding2D, MaxPooling2D, AveragePooling2D, Flatten, Dense, GlobalAveragePooling2D, \
    GlobalMaxPooling2D
...
def build_unet(backbone, classes, last_block_filters, skip_layers,
    ...
    skip_layers = ([l for l in skip_layers])
...
def UResNet50(input_shape=(None, None, 3), classes=1, decoder_filters=32, decoder_block_type='upsampling',
              input_tensor=None, activation='sigmoid'):
    backbone = ResNet50(include_top=False, input_shape=input_shape, input_tensor=input_tensor, weights=None)
    skip_connections = list([141, 79, 37, 4])  # for resnet 50
...
def get_model(input_shape, dropout=0):
    return UResNet50(input_shape=input_shape)
